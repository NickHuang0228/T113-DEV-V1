...
## 2026-09-20 · 讀完四張圖:腳位表帶出 placement 問題,而且 1.8V 要拆成兩條

四項待辦全部完成。**都是圖不是表,文字層抽不出來,只能 render 成高 dpi 圖片用看的。**
這正是專案規則「定位靠文字,定案靠看圖」的後半段。

### Figure 6 (p.18) — D[35:0] 的腳號,以及一個 placement 問題

原本 Table 3 只給「D[35:0] = pin 57-74, 78, 80-96」的範圍,**不能用連號推**
—— 因為 pin 79 (CLK) 夾在中間。開圖抄出來:

```
右側  57 D35 → 74 D18   （D 索引遞減）
頂側  78 D17 · 79 CLK · 80 D16 → 96 D0 · 97 DE · 98 HSYNC
左側  2 VSYNC
```
...
```
ADV7511 頂側 (81~94)   B[5:0] + G[5:0]   = PD0 ~ PD11
        頂側 (79/97/98) CLK / DE / HSYNC = PD18/19/20
        右側 (69~74)   R[5:0]            = PD12 ~ PD17
...
```
→ 18 條資料要分成「12 條進頂邊 + 6 條進右邊」兩束
→ VSYNC 一條要繞到晶片左側,是全組最長的一條
→ ADV7511 的旋轉角度直接決定這三束會不會打架
```

好消息:**MIPI 共用的 PD0~PD9 全部落在頂側 pin 83~94,10 顆 0Ω 可以集中在頂邊。**
對「0Ω 貼著主幹」那條規則是有利的。

還有一個順手的發現:**未用的 D[35:24] 正好是 pin 57~68,右側連續 12 支**,接 GND 很好走。

### Figure 23 (p.50) — 三組不是「一個名字一組」

我原本以為五個域分三組會是某種平均分法。實際是:

```
組1  DVDD                 pin 1, 19, 49, 76, 77
組2  AVDD + PVDD          pin 29,34,41 + 24,25
組3  PLVDD + BGVDD        pin 21 + 26
```

**AVDD 跟 PVDD 併一組、PLVDD 跟 BGVDD 併一組** —— 這種分法我猜不出來,只能看圖。
...
**ADV7511 要自己專屬的 1.8V LDO,不跟 SoC 共用。**

這句話在 §6.8 的內文裡,不在 §7 layout 建議那一節 —— 我前一輪只讀了 §7,漏掉。
**規格分散在不同章節,只讀「看起來相關」的那節會漏。**

連帶:

```
v0.6   一顆 600mA LDO 撐 232mA(SoC 52 + ADV7511 180)
v0.7   兩顆:SoC 側 ~52mA(內建 LDOA 就夠) + ADV7511 專屬 ~180mA(AP2112K)
```

**反而讓選料變簡單了。** 而且多花的成本是 $0.17。

### Figure 24 (p.52) — 最嚴的雜訊上限剛好落在 DC-DC 的頻段
...
1~20 kHz         約 28.5 mV
100 kHz          約 9.5 mV
200k~300 kHz     約 1.3 mV      ★ 最嚴
300k~1 MHz       約 1.5~1.7 mV
10 MHz           約 5.5 mV
```
...
```
為什麼要 LDO 不能直接用 DC-DC  → 切換漣波就在最嚴的那段
為什麼 LDO 之後還要加 LC       → 同上,兩層都不能省
為什麼要專屬 LDO 不能共用       → 共用的話 SoC 側的負載跳變會打進這個頻段
```

**三個「為什麼」原本是三條獨立的規定,看完曲線才知道是同一個原因。**

### Figure 27 (p.55) — 完整參考原理圖

INT / SDA / SCL / DDC / CEC / HEAC / R_EXT 的接法全在一張圖上,照抄即可。

⚠ 圖上 INT 與 SDA/SCL 的上拉畫 **1.8V**,因為 ADI 的範例主機是 1.8V。
Table 3 寫的是「上拉到 **MCU 的 IO 電源**」,§7.4 寫「1.8V **或** 3.3V」。
**本板 T113 的 PD/PG bank 是 3.3V,所以上拉到 3.3V。**

**參考原理圖上的電壓值是那塊板的,不是規格。** 差點又照抄。

### 剩下的

```
[ ] 未用 D 腳接 GND —— datasheet 未明說
     它們是輸入腳(VIL -0.3~0.7V),接 GND 電氣上安全
     ★ 已在文件裡標成「這是判斷,不是引用」
[ ] Figure 7 機構圖的圖形 —— 數字已抄,1:1 列印比對時再開原檔
[ ] ADV7511 Programming Guide(2.8MB)—— driver 移植階段才需要
```

### 方法筆記

**render 成圖片來讀,dpi 要夠。** 這次用 650 dpi 裁切局部,腳位標籤才看得清楚;
300 dpi 整頁看曲線圖夠用,但看不清 100 支腳的標籤。

```python
pg.get_pixmap(dpi=650, clip=fitz.Rect(x0,y0,x1,y1)).save("crop.png")
```

先用 `get_text("words")` 拿到座標找出圖的範圍,再裁切 render ——
**用文字定位,用圖片定案。** 這條規則第三次證明有效。

---

## 2026-09-21 · 階段 1 開工:T113-S3 符號,以及四處會讓人接錯的非連號

### 先更正一件事:PDF 其實沒進版控

昨天說「ADV7511 原始 PDF 已入庫」—— **錯的**。
`.gitignore` 第一條就是 `*.pdf`,而 `docs/reference/README.md` 早就定了政策:
「大型 PDF 不進版控,這裡記錄每份文件的來源,換機器時照這份重新取得。」

檔案在磁碟上,但 `git ls-files | grep pdf` 是空的。

已補上 ADV7511 的來源條目與章節速查,並註明 ADI 封鎖自動下載、必須手動按下載鈕。
數字摘要 `ADV7511KSTZ_extracted.md` 有進版控,所以換機器就算一時拿不到 PDF 也不會卡住。

**教訓:專案自己已經有的政策,我沒去讀就宣告了結果。**
`.gitignore` 和 `reference/README.md` 都在那裡。

### 符號:128 腳只能用看的

Figure 7-1 Pin Map 的標籤**是向量圖不是文字** —— 整頁只抽得到 41 個字。
`get_text()` 拿不到任何腳位名稱。

作法是分四邊裁切、900 dpi render、逐腳目視抄。
頂排和底排的標籤旋轉 90°,300 dpi 糊成一團,拉到 700~900 dpi 才清楚。

我第一次算裁切座標還算錯了(把顯示尺寸當成原始尺寸,少乘 1.754),
render 出來是一段內文。**座標換算要記得 Read 工具回報的是縮放後的尺寸。**

### ★ 四處非連號 —— 這才是這次真正的收穫

抄的時候最容易「看到 PD10、PD11 就以為後面是 PD12、PD13」。實際上:

```
1. PD12/PD13 對調      pin 69=PD13, pin 70=PD12
2. PG 頂排亂序          118=PG1, 119=PG2, 120=PG0, 122=PG5, 123=PG4
3. PE 底排亂序          幾乎整段遞減,而 PE0/PE1 被放到最後(44/45)
4. USB0 與 USB1 的 DP/DM 順序相反
                       112=USB1-DP, 113=USB1-DM
                       114=USB0-DM, 115=USB0-DP
```

**第 1 項最危險** —— PD12/PD13 是 RGB 的 R0/R1,接反紅色會少兩個 bit 且錯位,
而原理圖上看起來完全正常。

**第 4 項是我原本完全不會想到要檢查的。** 兩組 USB 的極性排列是鏡像的,
如果照「USB0 怎麼接,USB1 照抄」的直覺做,差分對就反了。

### 驗證方式:跟 datasheet 自己的統計表對帳

DS §4.1 有一張 Pin Quantity 表(I/O 102 · NC 1 · Power 21 · Ground 1 · DDR Power 3)。
把抄出來的 129 筆按型別分類統計,**五項全中**。

```
Power = 21 這一項有個小關卡:要把 VRA1/VRA2 算進電源類(內部參考電壓輸出)
不算的話是 19,對不上 —— 反過來證明分類方式與原廠一致
```

再加上 1~128 無缺號無重複、名稱無重複、六個 GPIO bank 的索引各自連續。

⚠ **但只對總數不夠** —— 把兩支腳互換,總數和分類都不變。
所以「四處非連號」還是得靠逐腳目視,統計驗不出來。

### 不手畫符號

128 腳手輸入,錯一腳就是一條接錯的線,而且看不出來。改成:

```
CSV ──[gen_t113_symbol.py]──▶ .kicad_sym
 ▲                               │
 └── 目視核對機構圖               └── 回讀驗證,逐項比對
```

回讀驗證是從產生的 `.kicad_sym` 反解出每支腳的 `(number, name, type)`,
與 CSV 比對 —— 129/129 全中,無不符無多餘。

**「符號和資料不一致」這種錯誤現在不可能發生了。**

KiCad 10.0.6 的 `sym upgrade --force` 能存 = 格式合法;
`sym export svg` 七個單元都畫得出來 = 結構完整。

順帶一提,KiCad 10 把格式版本寫成 `20251024` / `generator_version "10.0"`,
產生器的版本字串已對齊,免得每次都被 upgrade 改一次。

---

## 2026-09-21 · 電源樹定案,以及把「你畫我看」變成可自動檢查的事

### 又一次「零件沒幫你做事」

001-power.md 的比較表寫 RY1303 的優點是「EN1/EN2/EN3 **內建順序**」。
拿到 datasheet 一看:
...
**三支 EN 是獨立輸入,沒有任何排序功能。** 時序要自己做。

錯誤來源是把「有三支 EN 腳」當成「有排序功能」。
**零件的彈性不等於零件幫你做事** —— 這跟前面「加警語不等於查證」是同一類毛病:
把「看起來像」當成「就是」。

### 文字比圖寬鬆 —— 只看圖會多綁一條不存在的限制

Figure 5-28 把 VCC-DRAM 畫在第一群組(T1 之前),看起來像硬體順序要求。
但 §5.12.1 的文字只寫:
...
前面一直在講「定位靠文字,定案靠看圖」,這次是反過來 ——
**圖是 example,文字才是 requirement。** 兩邊都要讀,而且要知道哪邊說了算。

### RESET:確認 MangoPi 沒滿足規格

開原理圖目視:`R74 10K 上拉 LDOA-OUT + C84 0.1µF`,RC = 1 ms。
規格要 T2 > 64 ms。**差 60 倍。**

001-power.md §3.4 當初的推測(「靠 soft-start 湊出來,或根本沒滿足但恰好能開機」)
現在有確切數字了:**它確實沒滿足。**

改用 APX803-29SAG-7(200 ms 延遲,$0.25),T2 = 195 ms,餘裕 3 倍。

⚠ 上拉要接 **+1V8_SOC 不是 +3V3** —— T113 的 RESET 是 1.8V 域輸入,
MangoPi 用 LDOA-OUT(1.8V)上拉可以佐證。APX803 是 open-drain,只負責拉低,
高電位由上拉電阻決定,所以電平相容沒問題。

### 順手補到一顆漏掉的必要元件

開 MangoPi 原理圖查 VDD18-DRAM 的時候,看到 pin 47 DZQ 接 **R42 240Ω 1%** 到地。

**我原本的元件清單裡完全沒有這顆。** DDR ZQ 校準電阻,少了 DDR3 不會動。

**去查 A 的時候順便看見 B** —— 這種收穫只有開原圖才有,
抽文字或搜關鍵字都不會撞到。

### 一個沒有完美解的取捨,記下來當嫌疑犯

規格寫 `VCC-RTC must be ramped no later than other power rails`。
但 +1V8_SOC 是從 +3V3 經 LDO 產生的,**必然晚數百 µs**。

```
改吃 5V     (5−1.8)×0.18 = 0.58W 在 SOT-25,θJA 250°C/W → ΔT 145°C,太燙
第四通道    沒有,RY1303 三路已用完
加一顆 DC-DC  為了數百 µs 多一顆 IC,不划算
```

**接受。** 依據是 MangoPi 同樣這樣接(VCC-RTC ← LDOA-OUT ← LDO-IN ← 3.3V)而能開機。

但這條寫進 011 §4.5,標題就叫「沒有完美解的取捨」——
**萬一開不了機,要記得它是嫌疑犯。** 不寫下來,三個月後只會在那邊瞎猜。

### 協作方式:把檢查變成自動的

Nick 要自己畫原理圖,我負責看。

**但「用眼睛看 128 隻腳」本來就是會失敗的事** —— 這整個專案已經證明過很多次了
(六組 EPAD、料號尾碼、R/B 標反、PD12/13 對調)。所以做了檢查器。

關鍵設計決定:**不自己解析 wire / junction / label 的幾何。**

```
kicad-cli sch export netlist  ──▶  讓 KiCad 自己算出 net
                                    再對 netlist 套規則
```

判斷「這條線有沒有連到那支腳」是最容易搞錯的部分,那個交給 KiCad。
我只做「這支腳在哪條 net 上、對不對」的比對 —— 這是純資料比對,不會錯。

規則只在相關元件存在時生效,所以只畫了電源頁不會被 ADV7511 的規則洗版。
每條警告都附設計文件出處,看到警告知道去哪查。
...
```
正:手寫一張 2 元件的測試圖 → 解析出 2 元件 3 net,規則通過
反:故意把 887R 改成 1K    → 「缺元件」警告正確觸發
```

順便確認了 netlist 的實際格式:net 名有 `/` 前綴要去掉,
`unconnected-(R1-Pad2)` 這種要排除,否則單腳 net 的檢查會誤報。

**不能讓 Nick 畫完一整頁才發現工具是壞的。**
...
```
階段 1  原理圖
  [x] T113-S3 符號(128 腳已核對,由 CSV 生成)
...
開 `hardware/T113-DEV-V1.kicad_pro`,符號庫已掛好。
畫一段跑一次 `python hardware/scripts/check_schematic.py`。

還沒答的(不阻塞畫圖,但下單前要補):

```
[ ] RY1303 的 EN 輸入漏電流 → 決定 47K 夠不夠小
[ ] 三顆電感選型 + 飽和電流 + LCSC 料況
[ ] AP2112K 的輸入/輸出電容要求
